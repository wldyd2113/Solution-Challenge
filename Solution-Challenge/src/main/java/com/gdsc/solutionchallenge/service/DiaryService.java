...
public class DiaryService {
    ...
    @Transactional
    public DiaryResponseDto writeDiary(DiaryRequestDto requestDto, Long memberId) {
        Member member = userRepository.findById(memberId)
                .orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다."));
        String memberDate = requestDto.getDate();
        if (diaryRepository.existsByDateAndMember(memberDate, member)) {
            throw new IllegalArgumentException("해당 날짜에는 이미 일기가 작성되었습니다.");
        }
        Diary diary = Diary.create(requestDto, member);
        Diary savedDiary = diaryRepository.save(diary);
        return DiaryResponseDto.of(savedDiary);
    }
    @Transactional
    public DiaryResponseDto getDiary(String date, Long memberId) {
        Member member = userRepository.findById(memberId)
                .orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다."));
    ...
    @Transactional
    public OldestDiaryResponseDto getOldestDiary(Long memberId, String emotion){
        Member loggedInMember = userRepository.findById(memberId)
                .orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다."));
        Diary oldestDiary = diaryRepository.findOldestDiary(loggedInMember, emotion)
    ...
    @Transactional
    public DiaryResponseDto writeMessage(MessageDto messageDto, Long diaryId, Long memberId) {
        Member member = userRepository.findById(memberId)
                .orElseThrow(() -> new IllegalArgumentException("사용자를 찾을 수 없습니다."));
        ...
        diary.setMessageLocation(member.getLocation());
    ...
    public Long getDiaryCount(Long memberId){
        return userRepository.countDiariesByMemberId(memberId);
    }
}
